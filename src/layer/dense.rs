...
impl Dense {
         ...
         , w_init: &'static str, b_init: &str) -> Dense
  {
    Dense {
      ...
      inputs:  Input{data: vec![initializations::get_initialization("zeros", Dim4::new(&[input_size, 1, 1, 1])).unwrap()] // z_{l-1}
                     , activation: vec![output_activation]},                                                              // activ{z_{l-1}}
...
impl Layer for Dense {
  ...
  fn forward(&mut self, inputs: &Input) -> Input {

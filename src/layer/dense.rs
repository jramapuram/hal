...
impl Layer for Dense
{
  ...
  fn backward(&self, params: Arc<Mutex<Params>>, delta: &Array) -> Array {
    ...
    ltex.deltas[0] = af::add(&ltex.deltas[0], &dw, false).unwrap();
    ltex.deltas[1] = af::add(&ltex.deltas[1], &db, false).unwrap();

    af::matmul(&delta_t, &ltex.weights[0], af::MatProp::NONE, af::MatProp::TRANS).unwrap()
  }
}
